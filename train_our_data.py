...
latents_cache = OUT_DIR / "spender_encodings.npz"
latents = None

if latents_cache.exists():
    cache = np.load(latents_cache)
    if cache["latents"].shape == (N, 6):
        latents = cache["latents"]
        log.info(f"Loaded cached encodings from {latents_cache}: {latents.shape}")
...
if latents is None:
    try:
        ...
        np.savez_compressed(latents_cache, latents=latents)
        log.info(f"Encoded {N:,} spectra in {time.time()-t0:.1f}s; cached to {latents_cache}")

    except ImportError:
        raise ImportError(
            "spender not importable. Install via: pip install spender\n"
            f"Or place cached encodings at: {latents_cache}"
        )
